...
import com.ADBMS.orderservice.dto.*;
...
public class OrderService {
    ...
    public Order cancelOrder(Long orderId,String userId) {
        ...
        // 1. Fetching order entity.
        OrderRequestDTO order = orderRepository.findById(orderId).orElse(null);
        if(fetchUserExistence(order.getUserID())){
        ...
        // 4. Increase the relevant quantities from the Inventory microservice.
        increaseInventoryQuantities(orderItems);
    ...
    private Boolean fetchUserExistence(String userID) {
        WebClient webClient = webClientBuilder.baseUrl("http://user-service").build();
        UserDTO userDTO = webClient.get()
                .uri("/api/users/{userID}", userID)
                .retrieve()
    ...
    private void increaseInventoryQuantities(List<OrderItem> orderItems) {
        WebClient webClient = webClientBuilder.baseUrl("http://inventory-service").build();
        List<Mono<Void>> updateRequests = new ArrayList<>();
        for (OrderItem orderItem : orderItems) {
            Mono<Void> updateRequest = webClient.put()
                    .uri("api/inventory/{productID}/quan-increase/{quantityToIncrease}", orderItem.getProductID(),orderItem.getQuantity())
                    .retrieve()
                    .onStatus(HttpStatusCode::is4xxClientError, clientResponse -> {
                        if (clientResponse.statusCode() == HttpStatus.NOT_FOUND) {
                            throw new ProductNotFoundException("Product not found with the productID : " + orderItem.getProductID());
